...
export default async function handler(req,res){
 ...
 try{
  ...
  const items=b.items.map(x=>{const p=pr.find(y=>y.id===Number(x.id));const qty=Math.max(1,Math.min(99,Number(x.qty)||0));return p?{id:p.id,name:p.name,qty,price:p.price}:null}).filter(Boolean);
  if(!items.length)return res.status(400).json({ok:false,message:"المنتجات المطلوبة غير متاحة."});
  const total=items.reduce((s,x)=>s+x.price*x.qty,0);
  const ro=await fetch(process.env.SUPABASE_URL+"/rest/v1/orders",{method:"POST",headers,body:JSON.stringify({name,phone,address,landmark,total,status:"جديد"})}).then(r=>r.json());
  const id=ro[0]?.id;if(!id)throw new Error("db");
  await fetch(process.env.SUPABASE_URL+"/rest/v1/order_items",{method:"POST",headers,body:JSON.stringify(items.map(x=>({order_id:id,product_id:x.id,product_name:x.name,qty:x.qty,price:x.price})))});
